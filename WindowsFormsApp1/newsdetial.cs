...
namespace news
{
     ...
     * 同时加载新闻
     */
    public newsdetail(string id, string name)
    {
            InitializeComponent();
            news_name = id;
            db = new DBHelper();
            db.Connection();
            user_name = name;
            newsdate = db.find_news(news_name);
            if (newsdate.Tables[0].Rows.Count > 0)//新闻存在
            {
                ...
                string pic= newsdate.Tables[0].Rows[0]["picture"].ToString();
                if(pic=="")
                {
                    pic = @"\pic\0.jpg";
                }
                ...
                commendData = db.find_commendBynewsname(news_name);
        ...
        private void add_comend_button_Click(object sender, EventArgs e)
        {
            ...
            Boolean success = db.addCommend(user_name, news_name, commend_text, DateTime.Now.ToString());
            if (success)
            {
        ...
        {
            Boolean success = db.dianzan(user_name, news_name);
            if(success)
            {
                MessageBox.Show("点赞成功");
            }
            else
                MessageBox.Show("点赞失败");
        }
    }
}
